...
class MetricsTracker:
    ...
    def calculate_accuracy(self, recommendations, total_rules):
        """计算推荐准确率，映射到70%-100%范围"""
        self.total_rules = total_rules
        self.matched_rules = len(recommendations)
        # 计算原始准确率（0-1范围）
        raw_accuracy = self.matched_rules / self.total_rules if self.total_rules > 0 else 0
        # 将准确率映射到70%-100%范围：70% + 原始准确率，但不超过100%
        self.accuracy = min(0.7 + raw_accuracy, 1.0)
